...
void build_psp_header(PSP_Header2 *psph, u8 *ebuf, int esize)
{
	...
	if ((int) (elf->e_shoff + shtab_size) > esize)
    {
		e_shnum = 0;
	}
    else
	...
	sh = find_section((char *) ".rodata.sceModuleInfo");
...
int sign_eboot(u8 *eboot, int eboot_size, int passes, u8 *seboot)
{
	...
	if (passes == 1)
	{
		printf("Packing and signing (%d pass)...\n", passes);
	}
	else
	{
		printf("Packing and signing (%d passes)...\n", passes);
	}
