...
const int kNumDivPassesMax = 16;
const UInt32 kNumTables = (1 << kNumDivPassesMax);

// [0, (1 << 32)); ratio/speed tradeoff; use big value for better compression ratio.
static UInt32 kFixedHuffmanCodeBlockSizeMax = (1 << 32);
// [1, (1 << 32)); ratio/speed tradeoff; use small value for better compression ratio.
static UInt32 kDivideCodeBlockSizeMin = 1;
// [1, (1 << 32)); ratio/speed tradeoff; use small value for better compression ratio.
static UInt32 kDivideBlockSizeMin = 1;

// [1, (1 << 32))
static const UInt32 kMaxUncompressedBlockSize = ((1 << 13) - 1) * 1;
// [kMatchMaxLen * 2, (1 << 32))
...
static const UInt32 kBlockUncompressedSizeThreshold = kMaxUncompressedBlockSize - kMatchMaxLen - kNumOpts;
...
NO_INLINE void CCoder::ModLevels(const Byte *levels, int numLevels, Byte *lens, const UInt32 *codes)
{
  int prevLen = 0xFF;
  int nextLen = levels[0];
  int count = 0;
  ...
  for (int n = 0; n < numLevels; n++)
  {
    ...
    if (count < minCount)
    {
      ...
      {
        lens[curLen] = nbr_bits(codes[curLen]);
      }
    }
    else if (curLen != 0)
    {
      ...
      lens[kTableLevelRepNumber] = nbr_bits(codes[kTableLevelRepNumber]);
    }
    else if (count <= 10)
    {
        lens[kTableLevel0Number] = nbr_bits(codes[kTableLevel0Number]);
    }
    else
    {
      lens[kTableLevel0Number2] = nbr_bits(codes[kTableLevel0Number2]);
    }
...
NO_INLINE void CCoder::LevelTableCode(const Byte *levels,
                                      int numLevels,
                                      Byte *lens,
                                      const UInt32 *codes)
{
...
NO_INLINE void Huffman_ReverseBits(UInt32 *codes, Byte *lens, UInt32 num)
{
  UInt32 x;
...
NO_INLINE void CCoder::WriteBlock()
{
  ...
  for (UInt32 i = 0; i < m_ValueIndex; i++)
  {
    ...
    {
      m_OutStream.WriteBits(mainCodes[codeValue.Pos],
                            m_NewLevels.litLenLevels[codeValue.Pos]);
    }
    else
    {
      UInt32 len = codeValue.Len;
      UInt32 lenSlot = g_LenSlots[len];
      m_OutStream.WriteBits(mainCodes[kSymbolMatch + lenSlot],
                            m_NewLevels.litLenLevels[kSymbolMatch + lenSlot]);
      ...
      m_OutStream.WriteBits(distCodes[posSlot],
                            m_NewLevels.distLevels[posSlot]);
      
      m_OutStream.WriteBits(dist - kDistStart[posSlot],
                            kDistDirectBits[posSlot]);
    }
  }
  m_OutStream.WriteBits(mainCodes[kSymbolEndOfBlock],
                        m_NewLevels.litLenLevels[kSymbolEndOfBlock]);
}
...
NO_INLINE UInt32 CCoder::TryDynBlock(int tableIndex, UInt32 numPasses, int Change)
{
...
NO_INLINE UInt32 CCoder::GetBlockPrice(int tableIndex, int numDivPasses)
{
  CTables &t = m_Tables[tableIndex];
  t.StaticMode = false;
  UInt32 price = TryDynBlock(tableIndex, m_NumPasses, FALSE);
...
void CCoder::CodeBlock(int tableIndex, bool finalBlock)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (t.StaticMode)
      {
        // Static mode is useless (for us at least)
      ...
      else
      {
        ...
        for (i = 0; i < kFixedMainTableSize; i++)
        {
            m_NewLevels.litLenLevels2[i] = m_NewLevels.litLenLevels[i];
        }
        for (i = 0; i < kSymbolEndOfBlock; i++)
        {
            m_NewLevels.litLenLevels2[i] = 0;
        }

        for (i = 0; i < m_ValueIndex; i++)
        {
            int test;
            int test2;
            CCodeValue &codeValue = m_Values[i];
            if (codeValue.IsLiteral())
            {
                UInt32 x = mainCodes[codeValue.Pos];
                x = ((x & 0x5555) << 1) | ((x & 0xAAAA) >> 1);
                x = ((x & 0x3333) << 2) | ((x & 0xCCCC) >> 2);
                x = ((x & 0x0F0F) << 4) | ((x & 0xF0F0) >> 4);
                int new_code = (((x & 0x00FF) << 8) | ((x & 0xFF00) >> 8)) >> (16 - m_NewLevels.litLenLevels[codeValue.Pos]);

                test = (char) nbr_bits(new_code);
                test2 = (char) m_NewLevels.litLenLevels2[codeValue.Pos];
                if(test > test2)
                {
                    m_NewLevels.litLenLevels2[codeValue.Pos] = test;
                }
            }
        }
