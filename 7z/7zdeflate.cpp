...
static unsigned int crc = 0xffffffffL;
                         ...
                         unsigned &out_size) throw ()
{
	...
	if(!compress_deflate_7z(in_data, in_size, passes, out_data, size))
    {
		return false;
	}
    dat_crc = updcrc(in_data, in_size);
	size = size + 10;
    footer = (unsigned int *) &out_data[size - 10];
    footer[0] = dat_crc;   //CRC32
	size = size + 4;
    footer[1] = in_size;
	size = size + 4;
